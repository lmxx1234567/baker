...
def get_plaintiff_info_v1(lines: List[str]) -> List[dict]:
    ...
    for line in lines:
        if line == '\n':
            continue
        line = re.sub(r'反诉被告|反诉原告','',line)
        if "原告" in line and re.search(r'审理终结|纠纷', line) is not None:
            break
        if re.search(r'判令被告支付原告',line) is not None:
            break
        line=re.sub(r'[？?!！]','',line)
...
def get_defendant_info_v1(lines: List[str]) -> List[dict]:
    ...
    for line in lines:
        if line == '\n':
            continue
        line = re.sub(r'反诉被告|反诉原告|\n','',line)
        if "原告" in line and re.search(r'审理终结|纠纷|本案', line) is not None:
            break
        if stop_searching:
            break
        line=re.sub(r'[？?!！]','',line)
        if similar(line, '委托诉讼代理人') > 0.5:
            ...
            for seg in seg_list:
                if seg.flag == 'PER' or seg.flag == 'nr':
                    ...
                    defendant_agent = re.sub(r'：|；|。|（|）|(，男)|(，女)|，', '', seg.word)
                ...
                for seg in seg_list:
                    if seg.flag == 'ORG' and "律" in seg.word:  # 没准叫律所
                        ...
                        if law_firm[-2:] == "律师":  # 删掉结尾的‘律师’
                            law_firm = law_firm[0:-2]
                        break
            if len(defendant_agent)>3 and (defendant_agent[-1] == '男' or defendant_agent[-1] == '女'):
                defendant_agent = re.sub(r'男|女', '', defendant_agent)
            if '共同' in line:
        ...
        # if '被告' in line:
            ...
            if len(line) < 10:
                ...
                continue
            if len(re.split(r'。', line)) == 4:
                tmpline = re.split(r'。', line)
                tmp01 = re.search(r'被告',tmpline[0])
                tmp02 = re.search(r'公司',tmpline[0])
                tmp1 = re.search(r'住',tmpline[1])
                tmp2 = re.search(r'统一社会信用',tmpline[2])
                if (tmp01 is not None) and (tmp02 is not None) and (tmp1 is not None) and (tmp2 is not None):
                    ...
                    continue
            if (len(re.split(r'。', line)) <= 3 and (re.search(r'住|地址|省|市', line) is not None or '保险股份有限公司' in line)):
                tmp_names = re.split(r'[被告：:，,。男女]', line)
                for tmp_name in tmp_names:
                    if tmp_name != '':
                        defendant_value = tmp_name
                ...
                if not skip_it:
                    for seg in seg_list:
                        if not skip_it and seg.flag == 'nr':
                            defendant_value = re.sub(r'[，：；。]', '', seg.word)
                            break
            if defendant_value is not None and defendant_value != "汉族" and defendant_value != "回族" and defendant_value != '\n':
